...
local function get_jdtls_paths()
  ...
  ---
  path.runtimes = {}
  local java17_path = vim.env.JAVA_HOME
  if java17_path and java17_path ~= "" then
  ...
  local java21_path = vim.env.JAVA21_HOME
  if java21_path and java21_path ~= "" then
    table.insert(path.runtimes, {
      name = "JavaSE-21",
      path = java21_path,
    })
...
local function jdtls_setup(_)
  ...
  if vim.env.JAVA21_HOME and vim.fn.executable(vim.env.JAVA21_HOME .. "/bin/java") == 1 then
    jcmd = vim.env.JAVA21_HOME .. "/bin/java"
  else
    local os_name = vim.loop.os_uname().sysname
    if os_name == "Linux" then
      local g1 = vim.fn.glob("/usr/lib/jvm/*21*/bin/java")
      if g1 ~= "" then
        jcmd = vim.split(g1, "\n")[1]
      else
        local g2 = vim.fn.glob("/nix/store/*openjdk-21*/bin/java")
        if g2 ~= "" then
          jcmd = vim.split(g2, "\n")[1]
        end
      end
